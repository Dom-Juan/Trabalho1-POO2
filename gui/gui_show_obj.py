...
def show_users_broker(user_list: list) -> None:
    ...
    layout.append(
        [sg.Column(Col, size=(640, 800), element_justification='c', scrollable=True, vertical_scroll_only=True)])
    layout.append([sg.Button('Sair', size=(15, 1), button_color=('white', 'firebrick3'))])
    window = sg.Window("Mostrando Todos os Corretores", layout, element_justification='c', resizable=True,
                       finalize=True,
                       modal=True)
    window.TKroot.minsize(320, 240)
    while True:
...
def show_all_sales_and_profit_month(obj_list) -> object:
    ...
    window = sg.Window("Mostrar vendas e lucro em data", layout, element_justification='c', resizable=True,
                       margins=(5, 5))
    while True:
        event, values = window.read(close=True)
...
def show_late_rentals_clients(rental_list: list):
    ...
    for c in clients:
        Col.append([sg.HSeparator()])
        Col.append([
            sg.Text('Cliente em atraso:', pad=(5, 5), size=(20, 1)),  # Label
            sg.Text(str(c.name), pad=(5, 5), size=(45, 1))  # Valor do obj.
        ])
        layout = []
        layout.append([sg.Text('Mostrando informações', pad=(5, 5), size=(25, 1))])
        layout.append(
            [sg.Column(Col, size=(640, 800), element_justification='c', scrollable=True, vertical_scroll_only=True)])
        layout.append([sg.Button('Sair', size=(15, 1), button_color=('white', 'firebrick3'))])
        window = sg.Window("Mostrando Clientes com aluguel em atraso", layout, element_justification='c', resizable=True,
                           finalize=True,
                           modal=True)
        window.TKroot.minsize(320, 240)
        while True:
